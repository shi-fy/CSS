...
class Dictionary(object):
    def __init__(self, word2idx=None, idx2word=None):
        # word2idx是一个字典，其中的元素是以 单词：在单词列表中的位置 的形式存在的，即为每个单词生成一个编号
        if word2idx is None:
            word2idx = {}
        # idx2word是一个存放单词的列表，
        if idx2word is None:
    ...
    # 标记句子中的单词，此方法中的add_word是一个布尔类型的，表示是否往字典中添加词
    def tokenize(self, sentence, add_word):
        ...
        # 如果列表中没有此单词且add_word为false时为该单词编号，但不往列表中添加
        else:
            for w in words:
    ...
    # 往单词列表中添加不重复的单词，并生成编号，最终返回这个编号。如果要添加的单词是重复的，直接返回这个单词的编号
...
# 将答案中的问题id、和图片id删除，并生成一个entry字典
def _create_entry(img_idx, question, answer):
    ...
    # entry中包含问题id、图片id、问题内容、答案以及image_idx：用于检索图像或特征
    return entry
