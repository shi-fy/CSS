...
def filter_answers(answers_dset, min_occurence):
    ...
    # 返回值的形式为 答案：答案为此答案的问题的id的集合
    return occurence


# 创建答案标签，同时将创建label2ans.pkl文件
...
def compute_target(answers_dset, ans2label, name, cache_root):
    ...
    ***answers_dset should be preprocessed***
    ...
    for ans_entry in answers_dset:
        ...
        labels = []
        scores = []
        # 将答案集中的答案的标签加入标签列表中，并根据其出现的次数为其分配软得分，加入得分列表中
        for answer in answer_count:
            ...
            scores.append(score)
        # labels_counts中的数据形式为 答案标签：答案出现次数
        label_counts = {}
